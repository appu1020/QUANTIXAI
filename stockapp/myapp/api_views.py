...
def health_check_comprehensive(request):
    """
    Comprehensive health check verifying Database, TensorFlow, Models, RAG, Static files, and Prediction engine.
    """
    from .services.model_manager import ModelManager
    from django.conf import settings
    from pathlib import Path
    ...
    try:
        ...
        health_status["tensorflow"] = f"ok (v{tf_version})"
    except Exception as e:
        health_status["tensorflow"] = f"error: {str(e)}"
        health_status["status"] = "warning"

    # 3. Models & Prediction Engine
    try:
        ModelManager.initialize(MODEL_DIR)
        models = ModelManager.get_models()
        scaler = ModelManager.get_scaler()
        health_status["models"] = "ok" if models and scaler else "missing"
        health_status["prediction_engine"] = "ok" if models and scaler else "missing_dependencies"
    except Exception as e:
        health_status["models"] = f"error: {str(e)}"
        health_status["prediction_engine"] = "unavailable"
        health_status["status"] = "warning"

    # 4. RAG
    try:
        pipeline = get_pipeline()
        health_status["rag"] = "ok" if pipeline else "uninitialized"
    except Exception as e:
    ...
    try:
        ...
        if static_root and Path(static_root).exists():
            ...
            health_status["static_files"] = "warning (not collected or path invalid)"
            # It might be served directly via whitenoise from STATICFILES_DIRS during dev
    except Exception as e:
        health_status["static_files"] = f"error: {str(e)}"
        
    # If anything critical is missing but app runs, return 200 with warning
    # so we don't crash HF Spaces healthcheck
